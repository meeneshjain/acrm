...
name(){return"<a href='https://www.adminer.org/'".target_blank()." id='h1'>Database</a>";}function
credentials(){return
...
head(){return
true;}function
css(){$J=array();$Rc="adminer_black.css";if(file_exists($Rc))$J[]=$Rc;return$J;}function
...
';}elseif(isset($_GET["dump"])){$a=$_GET["dump"];if($_POST&&!$n){$Bb="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$z)$Bb.="&$z=".urlencode($_POST[$z]);cookie("adminer_export",substr($Bb,1));$T=array_flip((array)$_POST["tables"])+array_flip((array)$_POST["data"]);$Gc=dump_headers((count($T)==1?key($T):DB),(DB==""||count($T)>1));$Qd=preg_match('~sql~',$_POST["format"]);if($Qd){echo"-- db $ia ".$cc[DRIVER]." dump\n\n";if($y=="sql"){echo"SET NAMES utf8;
SET time_zone = '+00:00';
